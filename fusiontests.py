...
patientId = "FusionTests"
patientName = "Fusion^Tests"
pathName = patientId.lower()
...
def build_orientation(patient_position, column_direction, row_direction, frame_of_reference_uid = None):
    sb = StudyBuilder(patient_position=patient_position, patient_id=patientId, patients_name=patientName, patients_birthdate = "20121212")
...
for o in orientations:
	for p in patientpositions:
                FoR += 0.1
                FoRstring = str(FoR)
	        sb = build_orientation(p, o[0], o[1], FoRstring)
		sbs.append(sb)
                #FoR = sbs[0].current_study['FrameofReferenceUID']
    		d = pathName + "/" + p + "/" + "%s%s%s%s%s%s" % tuple(x for y in o for x in y)
		os.makedirs(d)
		sb.write(d)
